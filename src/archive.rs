...
impl Archive {
    ...
    /// Counts every archived message and sums downloaded `media.size` by kind.
    /// GIFs (`kind=video` and `gif=1`) share the sticker bucket. Weight comes
    /// from persisted WhatsApp sizes, not from `stat` on disk.
...
pub(crate) mod tests {
    ...
    fn read_positions_and_pending_sync_survive_reopening_the_archive() {
        let dir = std::env::temp_dir().join(format!("whatsfast-read-test-{}", std::process::id()));

...
the post-build kill covers every `whatsfast.exe` (including an installer
copy). Do not use `cargo watch -x run` here.
...
- `src/updates/` downloads verified GitHub releases and hands installation to a
  helper from a one-click Update toast, or on the next process start when
  auto-download is on and a `.whatsfast-pending` payload is still newer.
  Closing to the tray is not a restart. Keep package-manager detection, asset
...
- Interface text goes through `src/i18n` (`i18n::t`, `i18n::f`, `i18n::count`);
  never paint a literal. Tables in `src/i18n/{key,en,es}.rs` are generated by
  `AGENTS/i18n-gen.py` from `AGENTS/i18n-strings.json` (the source of truth for
  rows and call-site replacements). After touching strings, run that generator
  and `AGENTS/i18n-residual.py`, which lists English text left on a page.
  `tests/i18n_es.rs` renders every demo page in Spanish and fails on leftovers.
